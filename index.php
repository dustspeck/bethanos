	...
	<script>
		...
		*/
		...
		if( /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) ) {
			...
			UA = "isPC";
			alert("Warning! This game is designed for mobile screen.");
		}
		//console.log(UA);
		...
		function endScreen(){
			...
			if(level<=10){
				...
				//write to ledger
				var raw =  regno.toString()+"|lost on level "+level+"|";
				var entrydata= {param:raw};
				console.log(entrydata);
				request = $.ajax({
					type:'POST',
					data: entrydata,
					url: 'ledger/write.php',
				});	
...
// 				stage.addChild(btnTick);
		...
		function startscreen(){
			...
			function greet2(){
				...
				btnPlay.on("pointertap", function(){
					...
					while(regno==null||regno==""){
						regno=prompt("Enter your registration number.");
					}

					if(regno!=null||regno!=""){ startGame();}
				});
	...
	function writeRecord(){
		var raw = name.toString() + "|" + Math.trunc(score).toString() + "|" + regno.toString();
		var entrydata= {param:raw};
		console.log(entrydata);
		request = $.ajax({
			type:'POST',
